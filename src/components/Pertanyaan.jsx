...
const Pertanyaan = () => {
  ...
  return (
    <div className=" max-w-5xl container mx-auto mt-20 text-white section">
      <h2 className="text-3xl font-semibold mb-4">FAQ</h2>
      <div className="space-y-10 mt-10  rounded-lg shadow-orange-300 ">
        {questions.map((q, index) => (
          <div key={q.id} className="border-b-2 pb-2 border-green-500/30" data-aos="fade-up" data-aos-delay={`${index * 100}`}>
            <button
            ...
            {openQuestion === q.id && <p className="mt-2 text-[.9rem] pt-3 text-slate-300">{q.answer}</p>}
          </div>
        ))}
      </div>
    </div>
